...
GLDebugDrawer::GLDebugDrawer() {
    shader_programme = Shader("assets/shaders/debug.vert", "assets/shaders/debug.frag");
    viewLoc = glGetUniformLocation(shader_programme.ID, "view");
    projLoc = glGetUniformLocation(shader_programme.ID, "projection");
}
GLDebugDrawer::~GLDebugDrawer() {}
...
void GLDebugDrawer::drawLines() {
    int n = lines.size();
    GLuint* indices = new GLuint[n * 2];
    GLfloat* vertices = new GLfloat[3 * n * 2];
    ...
    //use program
    glUseProgram(shader_programme.ID);
